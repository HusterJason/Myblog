...
def add_post(request):  # 前端发布文章
    form = PostForm()  # form 变量首先被初始化为一个空的 PostForm 实例
    if request.method == 'POST':
        form = PostForm(request.POST)
        if form.is_valid():
            new_post = form.save(commit=False)
            new_post.owner = request.user  # 假设你有一个 author 字段
            new_post.save()
...
def friend_link(request):
    # 这里可以添加一些逻辑，比如获取数据等
    return render(request, 'users/friend_link.html')


def published_posts(request):
    user = request.user
    posts = Post.objects.filter(owner=user)
    return render(request, 'users/published_posts.html', {'posts': posts})
